var currentLangue = 'ObjectiveC' //语言
 ...
 * model信息
 */
function LYModelInfoMeta(json, clsName) {
  idCounter++
  ...
  this.className = clsName ? clsName : 'Model' //class 名称
  this.json = json //对应的json
  this.propertyInfoArray = [] //属性信息，格式[LYModelPropertyInfoMeta对象]
  this.fileHandler = {} //文件信息管理工具，LYFileInfoMetaHandler对象
  ...
  this.parseJSON = function (json) {
    ...
    for (var i in properties) {
      ...
      var propertyMeta = new LYModelPropertyInfoMeta(propertyName, value)
      ...
      if (propertyMeta.constructorType == 'object') {
        ...
        if (propertyMeta.genericClassName == 'object') {
          //Array里边是json(object)
          self.paresForObject(propertyMeta)
        }
      }
    ...
    //文件信息
    self.fileHandler = new LYFileInfoMetaHandler(self)
  }
  ...
  this.paresForObject = function (propertyMeta) {
    ...
    var value = propertyMeta.isContainer
      ? propertyMeta.value[0]
      : propertyMeta.value
    ...
    if (exist) {
      ...
      if (usingConfigJSON.modelNameHeader || usingConfigJSON.modelNameFooter) {
        var clsNameHeader = usingConfigJSON.modelNameHeader
          ? usingConfigJSON.modelNameHeader
          : ''
        var clsNameFooter = usingConfigJSON.modelNameFooter
          ? usingConfigJSON.modelNameFooter
          : ''
        clsName =
          clsNameHeader + propertyName.LYFirstUpperCase() + clsNameFooter
      ...
      var modelMeta = new LYModelInfoMeta(value, clsName)
 ...
 * json的valu值
 */
function LYModelPropertyInfoMeta(key, value) {
  this.name = key //属性名称
  ...
  this.pointerString = usingConfigJSON.pointerString
    ? usingConfigJSON.pointerString
    : '' //指针标记字符，OC里边为*
    ...
    if (value != null) {
      ...
      if (propertyType == 'object') {
        if (value.constructor == Date) {
          ...
          if (resultArr.length <= 0 && value.length > 0) {
            //Array里边是json(object)
            ...
            if (resultArr.length <= 0) {
              ...
              self.genericClassName = configPropertyJSON[tName]
                ? configPropertyJSON[tName]
                : ''
            }
          }
        }
  ...
  //model属性信息获取
  var pointerTypes = usingConfigJSON.pointerTypes
    ? usingConfigJSON.pointerTypes
    : []
  ...
  if (this.constructorType == 'string') {
    keyword = usingConfigJSON.propertyKeywords.string
      ? usingConfigJSON.propertyKeywords.string
      : keyword
  } else if (this.isPointer) {
    keyword = usingConfigJSON.propertyKeywords.pointer
      ? usingConfigJSON.propertyKeywords.pointer
      : keyword
  }
 ...
 * 文件管理工具
 */
function LYFileInfoMetaHandler(modelInfoMeta) {
  this.modelInfoMeta = modelInfoMeta
  this.filesArray = [] //文件信息数组，格式[LYFileInfoMeta对象]
...
LYCreateFileContentForAllClass = function (array) {
  //下面将分发到各个语言的creator
  if (currentLangue == 'ObjectiveC') {
    return LYCreateFileContentForAllClassForObjectiveC(array)
  } else if (currentLangue == 'Swift') {
    return LYCreateFileContentForAllClassForSwift(array)
  } else if (currentLangue == 'Java') {
    return LYCreateFileContentForAllClassForJava(array)
  }
}
 ...
 * 文件信息
 */
function LYFileInfoMeta(modelInfoMeta) {
...
LYJSONParse = function (json) {
  ...
  var modelMeta = new LYModelInfoMeta(json)
 ...
 * 首字母大写
 */
String.prototype.LYFirstUpperCase = function () {
